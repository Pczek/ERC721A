      ...
      context('with minted tokens', async function () {
        ...
        describe('exists', async function () {
          it('verifies valid tokens', async function () {
            for (let tokenId = 0 + startTokenId; tokenId < 6 + startTokenId; tokenId++) {
          ...
          it('verifies invalid tokens', async function () {
            expect(await this.erc721a.exists(6 + startTokenId)).to.be.false;
            if (startTokenId > 0){
        ...
        describe('balanceOf', async function () {
          ...
          it('throws an exception for the 0 address', async function () {
            await expect(this.erc721a.balanceOf(ZERO_ADDRESS))
        ...
        context('_totalMinted', async function () {
          ...
          it('_burn does not affect _totalMinted', async function () {
            const totalMintedBefore = await this.erc721a.totalMinted();
            const totalSupplyBefore = await this.erc721a.totalSupply();
            expect(totalMintedBefore).to.equal(totalSupplyBefore);
            await this.erc721a.directBurn(0 + startTokenId);
            expect(await this.erc721a.totalMinted()).to.equal(totalMintedBefore);
            expect((await this.erc721a.totalSupply()).add(1)).to.equal(totalMintedBefore);
          });
        });
        ...
        describe('ownerOf', async function () {
          it('returns the right owner', async function () {
            expect(await this.erc721a.ownerOf(0 + startTokenId)).to.equal(this.addr1.address);
            expect(await this.erc721a.ownerOf(1 + startTokenId)).to.equal(this.addr2.address);
            expect(await this.erc721a.ownerOf(5 + startTokenId)).to.equal(this.addr3.address);
          });

          it('reverts for an invalid token', async function () {
            await expect(this.erc721a.ownerOf(10 + startTokenId))
              .to.be.revertedWith('OwnerQueryForNonexistentToken');
          });
        });

        describe('approve', async function () {
          const tokenId = 0 + startTokenId;
          const tokenId2 = 1 + startTokenId;
          ...
          it('rejects an invalid token owner', async function () {
            const query = this.erc721a.connect(this.addr1).approve(this.addr2.address, tokenId2);
            await expect(query).to.be.revertedWith('ApprovalToCurrentOwner');
          });

          it('rejects an unapproved caller', async function () {
            const query = this.erc721a.approve(this.addr2.address, tokenId);
            await expect(query).to.be.revertedWith('ApprovalCallerNotOwnerNorApproved');
          });

          it('does not get approved for invalid tokens', async function () {
            const query = this.erc721a.getApproved(10 + startTokenId);
            await expect(query).to.be.revertedWith('ApprovalQueryForNonexistentToken');
          });
        });
        ...
        describe('setApprovalForAll', async function () {
          ...
          it('sets rejects approvals for non msg senders', async function () {
            const query = this.erc721a.connect(this.addr1).setApprovalForAll(this.addr1.address, true);
            await expect(query).to.be.revertedWith('ApproveToCaller');
          });
        });

        context('test transfer functionality', function () {
          const testSuccessfulTransfer = function (transferFn) {
            const tokenId = 1 + startTokenId;
            let from;
            let to;
            ...
            describe('safeTransferFrom', function () {
              ...
              it('validates ERC721Received', async function () {
                await expect(this.transferTx)
                  ...
                  .withArgs(this.addr2.address, this.addr2.address, 1 + startTokenId, '0x', GAS_MAGIC_VALUE);
              });
            });
          });
      ...
      context('mint', async function () {
        ...
        describe('safeMint', function () {
          it('successfully mints a single token', async function () {
            ...
            await expect(mintTx).to.emit(this.erc721a, 'Transfer')
              .withArgs(ZERO_ADDRESS, this.receiver.address, 0 + startTokenId);
            await expect(mintTx)
              .to.emit(this.receiver, 'Received')
              .withArgs(this.owner.address, ZERO_ADDRESS, 0 + startTokenId, '0x', GAS_MAGIC_VALUE);
            expect(await this.erc721a.ownerOf(0 + startTokenId)).to.equal(this.receiver.address);
          });
          ...
          it('successfully mints multiple tokens', async function () {
            ...
            for (let tokenId = 0 + startTokenId; tokenId < 5 + startTokenId; tokenId++) {
              await expect(mintTx).to.emit(this.erc721a, 'Transfer')
        ...
        describe('mint', function () {
          ...
          it('does not revert for non-receivers', async function () {
            ...
            expect(await this.erc721a.ownerOf(0 + startTokenId)).to.equal(nonReceiver.address);
          });

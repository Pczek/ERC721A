      ...
      context('with minted tokens', async function () {
        ...
        describe('setOwnersExplicit', async function () {
          ...
          it('handles single increment properly', async function () {
            ...
            expect(await this.token.nextOwnerToExplicitlySet()).to.equal((1 + startTokenId).toString());
          });

          it('properly sets the ownership of index 2 + _startTokenId()', async function () {
            let ownerAtTwo = await this.token.getOwnershipAt(2 + startTokenId);
            expect(ownerAtTwo[0]).to.equal(ZERO_ADDRESS);
            await this.token.setOwnersExplicit(3);
            ownerAtTwo = await this.token.getOwnershipAt(2 + startTokenId);
            expect(ownerAtTwo[0]).to.equal(this.addr2.address);
            expect(await this.token.nextOwnerToExplicitlySet()).to.equal((3 + startTokenId).toString());
          });
          ...
          it('sets all ownerships in multiple calls', async function () {
            ...
            expect(await this.token.nextOwnerToExplicitlySet()).to.equal((2 + startTokenId).toString());
            await this.token.setOwnersExplicit(1);
            expect(await this.token.nextOwnerToExplicitlySet()).to.equal((3 + startTokenId).toString());
            await this.token.setOwnersExplicit(3);
            for (let tokenId = 0 + startTokenId; tokenId < 6 + startTokenId; tokenId++) {
